...
import (
	...
	model "github.com/Go-GraphQL-SYSU/GraphQL-Service"
...
func peopleConvertion(p1 *modelsw.People) *model.People {
	p := &model.People{}
	p.ID = p1.ID
	p.Name = p1.Name
	p.Height = &p1.Height
...
func planetConvertion(p1 *modelsw.Planets) *model.Planets {
	p := &model.Planets{}
	p.ID = p1.ID
	p.Name = p1.Name
...
func filmConvertion(f1 *modelsw.Films) *model.Films {
	f := &model.Films{}
	f.ID = f1.ID
	f.Title = f1.Title
...
func speciesConvertion(s1 *modelsw.Species) *model.Species {
	s := &model.Species{}
	s.ID = s1.ID
	s.Name = &s1.Name
...
func starshipsConvertion(s1 *modelsw.Starships) *model.Starships {
	s := &model.Starships{}
	s.ID = s1.ID
	s.Name = &s1.Name
	s.Model = &s1.Model
...
func vehiclesConvertion(v1 *modelsw.Vehicles) *model.Vehicles {
	v := &model.Vehicles{}
	v.ID = v1.ID
	v.Name = &v1.Name
	v.Model = &v1.Model
...
func GetPeopleByID(ID string, db *bolt.DB) (error, *model.People) {
	var err error
	if db == nil {
...
func GetFilmByID(ID string, db *bolt.DB) (error, *model.Films) {
	var err error
	if db == nil {
		db, err = bolt.Open("data/data.db", 0600, nil)
		if err != nil {
...
func GetPlanetByID(ID string, db *bolt.DB) (error, *model.Planets) {
	var err error
	if db == nil {
		db, err = bolt.Open("data/data.db", 0600, nil)
		if err != nil {
...
func GetSpeciesByID(ID string, db *bolt.DB) (error, *model.Species) {
	var err error
	if db == nil {
		db, err = bolt.Open("data/data.db", 0600, nil)
		if err != nil {
...
func GetStarshipByID(ID string, db *bolt.DB) (error, *model.Starships) {
	var err error
	if db == nil {
		db, err = bolt.Open("data/data.db", 0600, nil)
		if err != nil {
...
func GetVehicleByID(ID string, db *bolt.DB) (error, *model.Vehicles) {
	var err error
	if db == nil {
		db, err = bolt.Open("data/data.db", 0600, nil)
		if err != nil {
